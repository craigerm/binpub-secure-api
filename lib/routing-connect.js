...
module.exports = function(app, controllerPath) {
  ...
  function buildServerArgs(routeName, middleware, routeHandler) {
    var arr = [];
    arr.push(routeName);
    _.each(middleware, function(item) {
      arr.push(item);
    });
    arr.push(routeHandler);
    return arr;
  }
  ...
  function addRoutingCall(app, verb, route, resource){
      ...
      var serverArgs = buildServerArgs(route, info.middleware, routeHandler);
      // Example: 
    ...
    del: function(route, resource){
      ...
      var idSuffix = options.id ? ':' + options.id : (':' + singularize(controllerName) + 'id');
        ...
        if (controller[action] !== undefined) {
          ...
          var middleware = getMiddleware(action, controller);
          var serverArgs = buildServerArgs(routeName, middleware, routeHandler);
          app[mapping.verb].apply(app, serverArgs);
				}
      }
